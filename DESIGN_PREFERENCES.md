...
- Structural radius:
  - all structural containers/cards use `20px` border radius consistently
- Global page heading spacing:
  - keep `40px` between the pills header row and page title on all pages
- Listing page titles:
  - section and author listing page titles use `40px` (`Oswald`)
  - normalize all page titles to `40px` (`Oswald`)
...
- Section listing pages (reachable from header pills):
  - remove article counter under the title
  - keep section names with proper accents (e.g., `A Selecção`, `Área Técnica`)
  - card grid spacing matches homepage `2x2` block spacing
- Archive page (`/arquivo/`):
  - background: `Blue` `#1F2937`
  - page title: `White` `#ECFDF5`, `40px`
  - content grouped by Year, then Month, both collapsible via arrow toggles
  - Year label: `24px`, `Green` `#16A34A`
  - Month label: `20px`, `Lighter Green` `#22C55E`
  - article row: category pill, title (`16px`), author (`13px`) + dot + date
